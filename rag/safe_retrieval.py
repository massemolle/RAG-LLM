"""
Safe Retrieval Index for RAG

BM25-based retrieval over the sanitized JSONL index produced by the
ingestion pipeline. Includes keyword gating and relevance thresholds
to limit noise in the retrieved context.
"""
...
STOP = {
    ...
    "and", "or", "what", "how", "why", "where", "when", "do", "does",
    "did", "with", "from", "by", "at", "as", "it", "this", "that",
    "these", "those", "be", "can",
}
...
class SafeIndex:
    """
    In-memory BM25 index over the rag/index/index.jsonl produced by
    the RAG ingestion pipeline.
    """

    def __init__(self, path="./rag/index/index.jsonl"):
        self.path = path
        self.records = []
        self.tokens = []
        self.bm25 = None
